...
interface LoginMutationLike {
  mutateAsync: (payload: { identifier: string; password: string }) => Promise<any>;
  isPending: boolean;
}
...
interface CredentialsLoginPageProps {
  ...
  loginMutation: LoginMutationLike;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || invalidCredentialsMessage);
      }
    } catch (err: any) {
      ...
      <div className="min-h-[80vh] flex items-center justify-center px-4 py-12 sm:px-6 lg:px-8 bg-cream">
        <div className="max-w-md w-full space-y-8 bg-white p-8 rounded-3xl border border-hairline shadow-resting relative overflow-hidden">
          <div className="text-center">
            ...
            <h2 className="text-2xl font-display font-bold text-forest">{title}</h2>
            <p className="mt-2 text-xs text-ink-soft max-w-sm mx-auto">{subtitle}</p>
          </div>

          <form className="mt-8 space-y-6" onSubmit={handleSubmit} id={`${idPrefix}-login-form`}>
            {notice}
            {error && (
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1.5">{identifierLabel}</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-ink-soft mb-1.5">{passwordLabel}</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none text-ink-soft/40">
                  ...
                  <input
                    ...
                    className="w-full bg-cream border border-hairline focus:border-forest/50 focus:bg-white rounded-xl pl-10 pr-10 py-3 text-sm text-ink outline-none transition-all placeholder:text-ink-faint/60"
                    placeholder="••••••••"
